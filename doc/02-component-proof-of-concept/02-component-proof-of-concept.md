...
The reason I decided to use dequeue on sequence as the design to move foward with is because it was
the most straight forward process to implement while also offering support for parameterized types. Dequeue on string would be difficult because how would non character or numerical values be stored. So for ease of implementation and overall applications I decided to build on sequence.

Dequeues also have many real world uses for modeling situations and working in programming so expanding the potential methods but building on a more flexible component will expand the amount of use cases.
